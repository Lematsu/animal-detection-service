...
def classify_images(images_dir, results_dic, model):
    ...
    """
    # Iterate through the dictionary processing each filename with the
    # classifier
    for filename, label in results_dic.items():
        classifier_label = classifier(images_dir + filename, model)
        classifier_label = classifier_label.lower().strip()
        # Append classifier label to dictionary value
        results_dic[filename].append(classifier_label)
        # Compare the pet image and classifer labels to determine if
        # they match
